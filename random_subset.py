...
def get_random_selection(fpath, outputfolder, num, verbose):
    list_of_files = Path(fpath).rglob('*.png')
    ...
    for fname in list_of_files[:num]:
        ...
        shutil.copy(fname, outputfolder.joinpath(str(fname.name).rsplit(".",2)[0]+".gt.txt"))
        shutil.copy(fname, outputfolder.joinpath(str(fname.name).rsplit(".",2)[0]+".json"))
        count += 1
        if verbose:
            print(f"{count} file copied:")
            print(f"{fname.name},{str(fname.name).rsplit('.',2)[0]+'.gt.txt'},{str(fname.name).rsplit('.',2)[0]+'.json'}")
